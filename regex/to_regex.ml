...
let of_expr_aux label_of_expr expr =
  ...
  let rec aux expr =
    ...
    | Pexp_ident id -> (
        ...
        | "anyA" -> AnyA
        | "allA" -> mk_all
        | _ -> RVar id#:Nt.Ty_unknown)
    | Pexp_constant _ | Pexp_array _ -> RConst (expr_to_constant expr)
...
let rec pprint_aux layout_ty layout_label = function
  ...
  | RApp { func; arg } ->
      ( spf "%s %s"
          ...
          (p_pprint layout_ty layout_label arg),
        false )
  | RLet { lhs; rhs; body } ->
      ( spf "let %s = %s in %s" lhs.x
          (p_pprint layout_ty layout_label rhs)
          (p_pprint layout_ty layout_label body),
        false )
  | Repeat (x, r) ->
...
let layout_regex_expr layout_ty layout_label r =
  layout_regex layout_ty layout_label r
...
let rec locally_rename ctx regex =
  ...
  | RVar x -> RVar x
  | RApp { func; arg } ->
      RApp { func = locally_rename ctx func; arg = locally_rename ctx arg }
  | RLet { lhs; rhs; body } ->
      RLet { lhs; rhs = locally_rename ctx rhs; body = locally_rename ctx body }
