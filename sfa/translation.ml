...
module MakeSFARegex (AB : ALPHABET) = struct
          ...
          | None ->
              ...
              let rest', m =
                EpsFA.CharMap.fold
                  (fun c s' (rest', m) ->
                    match c with
                    | EpsC.CEps -> (StateSet.elements s' @ rest', m)
                    | EpsC.CC c -> (rest', CharMap.add c s' m))
                  m ([], CharMap.empty)
              in
  ...
  let concat_eps_nfa (nfa1 : EpsFA.nfa) (nfa2 : EpsFA.nfa) : EpsFA.nfa =
    ...
    let next =
      StateSet.fold
        (fun final ->
          StateSet.fold (EpsFA.nfa_next_insert final EpsC.CEps) eps_nfa2.start)
  ...
  let kleene_eps_nfa (nfa : EpsFA.nfa) : EpsFA.nfa =
    ...
    let next =
      StateSet.fold
        (EpsFA.nfa_next_insert new_start EpsC.CEps)
        nfa.start nfa.next
    in
    let next = EpsFA.nfa_next_insert new_start EpsC.CEps new_final next in
    let next =
      StateSet.fold
        (fun final -> EpsFA.nfa_next_insert final EpsC.CEps new_start)
        nfa.finals next
    in
    let next =
      StateSet.fold
        (fun final -> EpsFA.nfa_next_insert final EpsC.CEps new_final)
        nfa.finals next
    in
    let eps_nfa : EpsFA.nfa =
      {
    ...
    let next =
      nfa_fold_transitions
        (fun (s, c, d) next ->
          ...
          if res then EpsFA.nfa_next_insert s (EpsC.CC c) d next
          else EpsFA.nfa_next_insert s EpsC.CEps d next)
        nfa StateMap.empty
    in
    let eps_nfa =
  ...
  let rec compile_regex_to_eps_nfa (r : CharSet.t regex) : EpsFA.nfa option =
    let res =
      match r with
      | Empty -> None
      ...
      | Seq [] -> None
      | Seq rs ->
          let rs' = List.filter_map compile_regex_to_eps_nfa rs in
  ...
  let rec layout_regex_lit = function
    | EqEps -> layout_regex Eps
    | EqEmp -> layout_regex Empty
    | EqChar r -> layout_regex (MultiChar r)
  ...
  let union_normal_form_to_regex ll =
    List.left_reduce [%here] alt @@ List.map (fun l -> seq l) ll

  let omit_layout_symbolic_trace rs =
    List.split_by "; " (function Star _ -> "□*" | _ as r -> layout_regex r) rs

  let layout_symbolic_trace rs = List.split_by "; " layout_regex rs
